...
def angle_from_dot_product(a,b):
    ...
    print "%f"%((1.0/(a_mag*b_mag))*( ax*bx+ay*by+az*bz ))
    theta=np.arccos( (1.0/(a_mag*b_mag))*( ax*bx+ay*by+az*bz ) )
...
# Base rotation is simply based on angle made within the x-y plane
    ...
    for j in range(1,max_iterations+1):
        ...
        q3 = -1.0 * angle_from_dot_product(v21, v32)
        q4 = -1.0 * angle_from_dot_product(v32, v43)
